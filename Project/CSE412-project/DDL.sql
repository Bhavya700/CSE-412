...
-- CREATE TABLES (Matching Phase 1 ER Diagram)
-- 1. COUNTRY Table
...
CREATE TABLE LEAGUE (
    ...
    Country_id INTEGER,
    CONSTRAINT fk_league_country FOREIGN KEY (Country_id) REFERENCES COUNTRY(Nationality_id) 
    ON DELETE SET NULL
);
...
CREATE TABLE CLUB (
    ...
    CONSTRAINT fk_club_league FOREIGN KEY (League_id) REFERENCES LEAGUE(League_id) 
    ON DELETE SET NULL
);
...
CREATE TABLE PLAYER (
    ...
    CONSTRAINT fk_player_nationality FOREIGN KEY (Nationality_id) REFERENCES COUNTRY(Nationality_id) 
    ON DELETE SET NULL,
    CONSTRAINT fk_player_club_team FOREIGN KEY (Club_team_id) REFERENCES CLUB(Club_team_id) 
    ON DELETE SET NULL
);
...
CREATE TABLE ADDITIONAL_INFO (
    ...
    CONSTRAINT fk_additional_info_player FOREIGN KEY (Player_id) REFERENCES PLAYER(Player_id) 
    ON DELETE CASCADE
);
...
CREATE TABLE RATINGS (
    ...
    CONSTRAINT fk_ratings_player FOREIGN KEY (Player_id) REFERENCES PLAYER(Player_id) 
    ON DELETE CASCADE
);
...
CREATE TABLE player_club (
    Player_id INTEGER PRIMARY KEY,
    Club_team_id INTEGER NOT NULL,  -- NOT NULL because we only insert players with clubs
    club_position VARCHAR(10),
    jersey_number SMALLINT CHECK (jersey_number >= 1 AND jersey_number <= 99),
    contract_until INTEGER,
    joined_date DATE,
    CONSTRAINT fk_player_club_player FOREIGN KEY (Player_id) REFERENCES PLAYER(Player_id) 
    ON DELETE CASCADE,
    CONSTRAINT fk_player_club_club FOREIGN KEY (Club_team_id) REFERENCES CLUB(Club_team_id) 
    ON DELETE CASCADE
);
...
CREATE TABLE player_country (
    Player_id INTEGER PRIMARY KEY,
    Nationality_id INTEGER NOT NULL,  -- NOT NULL because we only insert players with nationality
    nation_position VARCHAR(10),
    nation_jersey SMALLINT CHECK (nation_jersey >= 1 AND nation_jersey <= 99),
    CONSTRAINT fk_player_country_player FOREIGN KEY (Player_id) REFERENCES PLAYER(Player_id) 
    ON DELETE CASCADE,
    CONSTRAINT fk_player_country_country FOREIGN KEY (Nationality_id) REFERENCES COUNTRY(Nationality_id) 
    ON DELETE CASCADE
);
...
CREATE TABLE player_position (
    ...
    CONSTRAINT fk_player_position_player FOREIGN KEY (Player_id) REFERENCES PLAYER(Player_id) 
    ON DELETE CASCADE,
    CONSTRAINT fk_player_position_position FOREIGN KEY (Position_Code) REFERENCES POSITION(Position_Code) 
    ON DELETE CASCADE
);
